...
The continuous model showed more detailed separation of economic conditions. Its 5-state solution distinguished between two types of expansion, a broader slowdown/policy-easing regime, an elevated-stress/contraction regime, and the notable post-COVID rebound. States 3 and 4 captured recessionary conditions in different ways: State 3 contained 64.5% of all NBER recession months, while State 4 had the highest concentration of recession months in a state.

Overall, the two approaches captured major economic downturns, but the continuous model separated some periods into more specific economic environments. Its GDP results also followed an intuitive pattern, with stronger average growth in expansion states and weaker growth in slowdown and stress states. Furthermore, the discrete approach produced simpler and more interpretable state profiles, while the continuous approach preserved more information about the magnitude and joint behavior of the economic variables. Our results suggest that the two approaches provide complementary views of macroeconomic regimes rather than one clearly outperforming the other. Depending on the use case and degree of inter-pretability required, the continuous model could be more practical for deeper or continued analysis, and the discrete model’s results could be more easily communicated to a broader audience.
